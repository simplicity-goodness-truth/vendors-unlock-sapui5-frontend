...
sap.ui.define([
	...
	return BaseController.extend("ZAPPVNDUNLOCK.controller.Master", {
		...
		onInit: function() {
			...
			// Subscribing on events bus to understand if L1 approval was triggered
			this.oSelectedItem = null;

			var oEventBus = sap.ui.getCore().getEventBus();
			// 1. ChannelName, 2. EventName, 3. Function to be executed, 4. Listener
			oEventBus.subscribe("DetailAction", "onComiteeCentralApproval", this.onComiteeCentralApproval, this);

		},

		onComiteeCentralApproval: function(sChannel, sEvent, oData) {

			// Searching if updated incident is still in the list

		//	var oList = this.byId("list");
			var sObjectIds = [];

			for (var i = 0; i < this._oList.getItems().length; i++) {
				sObjectIds.push(this._oList.getItems()[i].getProperty('title'));
			}

			// If incident is still in the list we will put it as selected item

			if (sObjectIds.indexOf(oData.ObjectId) > 0) {

				this.oSelectedItem = this._oList.getItems()[sObjectIds.indexOf(oData.ObjectId)];
		...
		onUpdateFinished: function(oEvent) {
			...
			// If updated incident is still in the list, then put focus on it
			// Otherwise put a focus on a very first element
